...
class DDIMSampler(object):
    ...
    @torch.no_grad()
    def ddim_sampling(self, cond, shape, repeat_noise=False,
        ...
        if x_T is None:
            if repeat_noise:
                img = torch.randn((1, *shape[1:]), device=device).repeat(shape[0], *((1,) * (len(shape) - 1)))
            else:
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(img, cond, ts, index=index, repeat_noise=repeat_noise,
            ...
            if isinstance(c, dict):
                ...
                for k in c:
                    if isinstance(c[k], list):
                        ...
                        temp = {}
                        for key in c[k].keys():
                            temp[key] = torch.cat([unconditional_conditioning[k][key], c[k][key]])
                        c_in[k] = temp
                    else:
                        c_in[k] = torch.cat([
